...
Queues
    ...
    Queues are implemented in many ways, some based on arrays, and some based on
        linked structures

    rear: insertions are restricted to this end
    front: removal are restricted to the end

    two fundamental operations, add and pop
        push: adds item to the rear
        pop: removes item from the front
        peek: see the item at the from of the queue

    There are priority queues that schedule their elements using a rating scheme as
        well as FIFO. If two elements have same rating then they are scheduled in 
        ...
        matter when they are added t othe queue. (Dijkstra's shortest path algorithm)
"""
...
while True:
    ...
    menu_choice = int(input("Chose your option "))
    print()

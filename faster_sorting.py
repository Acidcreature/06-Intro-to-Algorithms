...
- Partition items in the list so that all items less than the pivot are moved to the
  ...
  pivot after the list is sorted could be at the end of the list or the beginning of the list
  depending on the size of the item.

- Divide and Conquer. Reapply the process recursively to the sublists formed by splitting
  the list at the pivot. One sublist consists of all items to the left of the pivot
...
# the first sublist lies btween low and middle, and the second between middle +1 and high
# - Set up index pointers to the first items in each sublist. There are low and middle + 1
# - starting with the first item in each sublist, repeatedly compare items. Compy the smaller
#   item from its sublist to the copyBuffer and advance to the next item in the sublist
# - copy portion of copyBuffer between low and high back to the corresponding positions in my_list
